...
BufferedDisplay::BufferedDisplay(OledDriver *displayDriver) {
  ...
  // FIXME: Add something to show if this fails!
}
...
void BufferedDisplay::writeCharacter(char character) {
  for(int i=0; i<FONT_WIDTH; i++) {
	byte c = pgm_read_byte (&(FONT[character - 0x20][i]));
	//~ writeSegment(FONT[character - 0x20][i]);
	writeSegment(c);
  }
}

void BufferedDisplay::writeString(const char *characters) {
